            ...
            <div class="mdl-grid tac">
                @foreach($popular_pizza->take(4)->get() as $key => $pizza)
                    <div class="mdl-cell mdl-cell--3-col item">
                        ...
                        data-info='[{"id":"{{ $pizza->id}}","size":"32","weigth":"{{ $pizza->getAttributeTranslate("weight_32")}}","category":"{{ $pizza->category()->first()->link }}","liters":null,"persons":null,"price":"{{ $pizza->getAttributeTranslate("price_32")}}"},{"id":"{{ $pizza->id}}","size":"40","weigth":"{{ $pizza->getAttributeTranslate("weight_40")}}","category":"{{ $pizza->category()->first()->link }}","liters":null,"persons":null,"price":"{{ $pizza->getAttributeTranslate("price_40")}}"}]'
                        ...
                        <div class="tac">
                            @if($pizza->getAttributeTranslate('size_32'))
                                <div class="item_button" data-pos="0">
                                    ...
                                    <button class="mdl-button mdl-js-button mdl-button--raised mdl-button--colored add_to_cart" data-category="{{ $pizza->category()->first()->link }}" data-size="32" data-id="{{ $pizza->id}}" data-price="{{ $pizza->getAttributeTranslate('price_32')}}" data-weight="{{ $pizza->getAttributeTranslate('weight_32')}}">{{ trans('base.order')}}</button>
                            ...
                            @if($pizza->getAttributeTranslate('size_40'))
                                <div class="item_button hidden" data-pos="1">
                                    ...
                                    <button class="mdl-button mdl-js-button mdl-button--raised mdl-button--colored add_to_cart"  data-category= "{{ $pizza->category()->first()->link }}" data-size="40" data-id="{{ $pizza->id}}" data-price="{{ $pizza->getAttributeTranslate('price_40')}}" data-weight="{{ $pizza->getAttributeTranslate('weight_40')}}">{{ trans('base.order')}}</button>
                                    <div class="item_button_done">
                                        <div><a href="{{ route('article_list', [null, 'cart', null]) }}">{{ trans('base.add_backet')}}</a></div>
                                    </div>
                                </div>
